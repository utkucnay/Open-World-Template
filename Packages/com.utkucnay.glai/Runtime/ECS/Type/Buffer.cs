...
namespace Glai.ECS
{
    public unsafe readonly struct Buffer<T> where T : unmanaged, IBufferComponent
    {
        ...
        public Buffer(void* slotPtr, int capacity)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Buffer(IntPtr slotPtr, int capacity)
        {
            lengthPtr = (int*)slotPtr;
            elementsPtr = (T*)((byte*)slotPtr + Core.BufferTypeInfo<T>.HeaderSize);
            this.capacity = capacity;
        ...
        {
            if (!TryAdd(item))
                throw new InvalidOperationException($"Buffer<{typeof(T).Name}> is full (capacity={capacity}).");
        }
