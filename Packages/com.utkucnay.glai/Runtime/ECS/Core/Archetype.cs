...
namespace Glai.ECS.Core
{
    ...
    {
        private ArchetypeConfig config;
        private FixedList<Chunk> chunkList;

        private FixedBitArray chunkIsAvailable;
        ...
        internal Archetype(in ArchetypeData archetypeData, MemoryStateHandle memoryStateHandle, MemoryState memoryState)
            : this(archetypeData, ArchetypeConfig.Default, memoryStateHandle, memoryState)
        {
        }

        internal Archetype(in ArchetypeData archetypeData, ArchetypeConfig config, MemoryStateHandle memoryStateHandle, MemoryState memoryState)
        {
            this.config = config;
            this.memoryStateHandle = memoryStateHandle;
            componentCount = archetypeData.ComponentCount;

            chunkList = new FixedList<Chunk>(config.MaxChunkCount, memoryStateHandle, memoryState);
            ...
            chunkIsAvailable = new FixedBitArray(config.MaxChunkCount, memoryStateHandle, memoryState);
            availableChunkIndices = new FixedStack<int>(config.MaxChunkCount, memoryStateHandle, memoryState);

            for (int i = 0; i < config.MaxChunkCount; i++)
        ...
        private Chunk CreateChunk(int index, MemoryState memoryState)
        {
            ...
            return new Chunk(new ChunkData
            {
                ...
                capacityBytes = config.ChunkCapacityBytes.Bytes,
                componentCount = dataComponentCount,
                componentSizes = dataSizes,
                alignment = config.ChunkAlignmentBytes
        ...
        public BufferRW<T> GetBufferRW<T>(EntityRecord entityRecord) where T : unmanaged, IBufferComponent
        {
            ...
            return new BufferRW<T>(slotPtr, capacity);
        }

        public BufferR<T> GetBufferR<T>(EntityRecord entityRecord) where T : unmanaged, IBufferComponent
        {
            int componentIndex = GetComponentStorageIndex(TypeId<T>.Id);
            if (componentIndex == -1)
                throw new InvalidOperationException($"Buffer component {typeof(T)} not found in archetype.");
            ...
            var (slotPtr, capacity) = chunkList[entityRecord.ChunkIndex].GetBuffer<T>(componentIndex, entityRecord.ComponentIndex);
            return new BufferR<T>(slotPtr, capacity);
        }
