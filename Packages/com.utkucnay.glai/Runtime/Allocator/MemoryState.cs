...
namespace Glai.Allocator
{
    ...
    {
        protected List<IAllocator> Allocators { get; private set; }
        protected int AllocatorCount { get => Allocators.Count; }

        public MemoryState()
        {
            Allocators = new List<IAllocator>(126);
        }
        ...
        protected MemoryStateHandle AddAllocator(IAllocator allocator)
        {
        ...
        public T Get<T>(MemoryStateHandle handle) where T : IAllocator
        {
            if (handle.Id != Id)
            {
